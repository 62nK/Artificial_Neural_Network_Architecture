...
public class Layer implements NetworkInterface{
    ...
    public double[] backPropagate(double learningRate, double[] deltaTargetObtained){
        double[] correctionArray = new double[dendritesCount];
        for(int index=0; index<neurons.length; index++){
            accumulate(correctionArray, neurons[index].backPropagate(learningRate, deltaTargetObtained[index]));
        }
