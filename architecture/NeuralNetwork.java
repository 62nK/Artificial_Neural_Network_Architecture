...
public class NeuralNetwork implements NetworkInterface {
    ...
    private void backPropagate(double learningRate, double[] target){
        ...
        for(int index=layers.length-1; index>0; index--){
            deltaTargetObtained = layers[index].backPropagate(learningRate, deltaTargetObtained);
        }
    }

    public void train(int epochs, double learningRate, ArrayList<InputTargetPair> inputTargetPairs){
        for(int epoch=1; epoch<=epochs; epoch++){
            if(DEBUG) System.out.println("Epoch: "+epoch+" ~ Learning Rate="+learningRate);
            networkError = 0;
            for(InputTargetPair inputTargetPair: inputTargetPairs) {
                if(inputTargetPair.getInputSize()==inNeuronCount && inputTargetPair.getTargetSize()==outNeuronCount) {
                    ...
                    backPropagate(learningRate<0?(1.0/(1+epoch)):learningRate, inputTargetPair.getTarget());
                }
            }
            networkError/=2;
        }
